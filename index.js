...
setInterval(() => {
  ...
  try {
    ...
    console.log('
 Chat log backup saved.');
  } catch (err) {
    console.error('
 Failed to write chat log backup:', err);
 ...
 DISABLE ghost mode via "New chat" workaround but works
    try {
      ...
      if (!ghostModeDisabled) {
        return res.send('Ghost mode disabled 
 (verified via "New chat").');
      }
      return res.send(`Ghost mode disabled 
 (via "New chat").`);
    } catch (err) {
      console.error('
 Error disabling ghost mode:', err);
      return res.status(500).send(`Failed to disable ghost mode: ${err.message}`);
    }
  } catch (err) {
    console.error('
 Error toggling ghost mode:', err);
...
app.post('/api/start-new-chat', validateToken, async (req, res) => {
 ...
 Failed to find or click the "New chat" button.');
    }
    await new Promise(resolve => setTimeout(resolve, 300));
    res.send('
...
app.post('/api/send-prompt', validateToken, async (req, res) => {
  ...
  try {
    await page.bringToFront();
    const inputSelectors = ['p[data-placeholder="Ask anything
"]', 'div.ProseMirror'];
    let inputHandle = null;
    for (const selector of inputSelectors) {
      try {
        await page.waitForSelector(selector, { timeout: 5000 }); // reduced for testing
...
async function clickFirstRemoveButton(page) {
 ...
 File removed!');
    ...
    console.error('
 Error in clickFirstRemoveButton:', err.message);
// remove all files
async function clickAllRemoveButtons(page) {
 ...
 No more files to remove.');
      ...
      console.log(`
 Removing ${removeButtons.length} file(s)...`);
      await clickFirstRemoveButton(page);
      await new Promise(resolve => setTimeout(resolve, 500));
    }
  } catch (err) {
    console.error('
 Error in clickAllRemoveButtons:', err.message);
app.post('/api/remove-file', validateToken, async (req, res) => {
 ...
 All files removed.');
    ...
    console.error('
 Error in /api/remove-file:', err.message);
...
app.post('/api/set-save-chat', validateToken, (req, res) => {
  ...
  if (enabled) {
    chatLogging.log = [];
  res.send(`
 Chat logging ${enabled ? 'enabled' : 'disabled'} using format: ${chosenFormat}`);
...
"]', 'div.ProseMirror'];
 ...
 Lumo response timed out');
    ...
    const output = ` 
 Hacker API message sent: ${cleanedMessage}  
 Lumo API responded: ${lumoResponse || 'No response detected'} `.trim();
    res.type('text/plain').send(output);
  } catch (err) {
    console.error('
 Error in send-hacker-prompt:', err);
  ...
  try {
    ...
    const sendSinglePrompt = async (prompt) => {
      ...
      const finalResponse = await page.waitForFunction(
        ...
        { timeout: 20000 }, // Increased overall timeout to 60 seconds
        previousResponse
      );
